function generareTeren1(target) {
    ...
    for(var j=1;j<7;j++){
        ...
        $("#id-"+i+j).attr('src','paginaUtilizator/assets/images/road.png');
        limite[i*10+j]=1;}
        else if(i*10+j!=target){
            $("#id-"+i+j).attr('src','paginaUtilizator/assets/images/iarba.png');
        }
    }
}
 ...
 if(  iarba[i][j]==0 && (i*10+j)!=target ){
  ...
  random1=target1;
   for(var j=1;j<=random;j++){
       ...
       limite[i*10+j]=1;
       if (j==random){
          $("#id-"+i+(j+1)).attr('src','paginaUtilizator/assets/images/curba.png');
           limite[i*10+j+1]=1;
       }
    }
for(i=2;i<=target1;i++){
          ...
          $("#id-"+i+(random+1)).attr('src','paginaUtilizator/assets/images/road1.png');
limite[i*10+(random+1)]=1;
          if (i==(target1)){
limite[i*10+random+1]=1;
            $("#id-"+i+(random+1)).attr('src','paginaUtilizator/assets/images/curba1.png');
          }
  }
if (random<=target2){
...
if(j!=target2)
              $("#id-"+target1+j).attr('src','paginaUtilizator/assets/images/road.png');
