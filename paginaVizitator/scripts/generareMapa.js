function generareTeren1(target) {
    ...
    for(var j=1;j<7;j++){
        ...
        $("#id-"+i+j).attr('src','paginaVizitator/assets/images/road.png');
        else if(i*10+j!=target){
            $("#id-"+i+j).attr('src','paginaVizitator/assets/images/iarba.png');
        }
    }
}
 ...
 if(  iarba[i][j]==0 && (i*10+j)!=target ){
  ...
  random1=target1;
   for(var j=1;j<=random;j++){
       ...
       $("#id-"+i+j).attr('src','paginaVizitator/assets/images/road.png');
       if (j==random){
          $("#id-"+i+(j+1)).attr('src','paginaVizitator/assets/images/curba.png');
       }
    }
for(i=2;i<=target1;i++){


          $("#id-"+i+(random+1)).attr('src','paginaVizitator/assets/images/road1.png');

          if (i==(target1)){

            $("#id-"+i+(random+1)).attr('src','paginaVizitator/assets/images/curba1.png');
          }
  }
if (random<=target2){
...
if(j!=target2)
              $("#id-"+target1+j).attr('src','paginaVizitator/assets/images/road.png');
